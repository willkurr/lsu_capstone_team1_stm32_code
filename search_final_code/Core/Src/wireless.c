...
uint8_t Wireless_ReadRegister(uint8_t reg) {
	...
	HAL_SPI_Receive(&hspi1, &receivedValue, 1, 100);
	CS_High();
...
void Wireless_TotalRegisterReset() {
	 ...
	 * CS_Low();
	 * HAL_SPI_Transmit(&hspi1, &command, 1, 100);
	 * HAL_SPI_Receive(&hspi1, &rxAddrP0Reg, 3, 100);
	 * CS_High();
	 */
...
void Wireless_WriteTXPayload(uint8_t *payload, int pldSize) {
	...
	CS_Low();
	HAL_SPI_Transmit(&hspi1, &wTxPldCommand, 1, 100);		//send write command
	HAL_SPI_Transmit(&hspi1, payload, pldSize, 100);		//write pay load into FIFO
	CS_High();
}
...
void Wireless_StartTxHandshake() {
	...
	Wireless_TransmitPld(connPld);							//write connection pay load into TX FIFO
...
void Wireless_UpdateRxAddress(uint8_t *rxUID, uint8_t dataPipe) {
	...
	HAL_SPI_Transmit(&hspi1, &command, 1, 100);				//send command to read address of desired data pipe

	for (int i = 0; i < 5; i++) {							//for loop to read and change address register
		HAL_SPI_Receive(&hspi1, &rxAddr[i], 1, 100);
		rxAddr[i] |= rxUID[i];
		rxAddr[i] &= rxUID[i];
	}
	CS_High();
	...
	for (int i = 0; i < size; i++) {						//for loop to write new address into register
		HAL_SPI_Transmit(&hspi1, &rxAddr[i], 1, 100);
	}
	CS_High();
}
/*
